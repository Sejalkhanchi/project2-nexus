<%-- 
    Document   : Signup
    Created on : 20 May, 2024, 2:38:40 PM
    Author     : Sejal
--%>
...
<%
    ...
    try {
        if (request.getParameter("btnSignup") != null) {
            ...
            conn = DriverManager.getConnection("jdbc:derby://localhost:1527/rms", "rms", "rms");

            // Check if email already exists
            stmt = conn.prepareStatement("SELECT * FROM signup WHERE email = ?");
            ...
            if (rs.next()) {
                ...
                if (x > 0) {
                    ...
                    out.print("<script>window.setTimeout(function(){ window.location.href = 'login.jsp'; }, 1000);</script>");
                } else {
        ...
        out.println("<script>alert('Database connection error: JDBC Driver not found.');</script>");
